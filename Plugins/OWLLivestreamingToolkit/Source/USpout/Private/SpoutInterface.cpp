...
FSpoutResource* GetRegisteredSpout(FString spoutName)
{
	auto Predicate = [&](const FSpoutResource InItem) { return InItem.Name == spoutName; };
...
FSpoutResource* PrepareSenderStructForSending(const FTexture2DRHIRef SrcTexture, FString spoutName)
{
	...
	FSpoutResource* Sender = GetRegisteredSpout(spoutName);
	if (Sender == nullptr)
	{
	...
	return GetRegisteredSpout(spoutName);
}
...
void USpoutInterface::Sender(FString spoutName, UTextureRenderTarget2D* textureRenderTarget2D)
{
	...
	ENQUEUE_RENDER_COMMAND(void)(
		[spoutName, textureRenderTarget2D](FRHICommandListImmediate& RHICmdList) {
			...
			if (textureRenderTarget2D == nullptr
				|| textureRenderTarget2D->Resource == nullptr
				|| textureRenderTarget2D->Resource->TextureRHI == nullptr
				|| textureRenderTarget2D->Resource->TextureRHI->GetTexture2D() == nullptr)
			{
				// this is a safety check in case the user is regenerating the resource or something
				UE_LOG(SpoutLog, Error, TEXT("Sender RenderTarget not initialised properly"));
				return;
			}
...
void USpoutInterface::Receiver(FString spoutName, UTextureRenderTarget2D* textureRenderTarget2D)
{
	if (textureRenderTarget2D == nullptr)
	{
	...
	if (textureRenderTarget2D->RenderTargetFormat != RTF_RGBA8_SRGB)
	{
	...
	ENQUEUE_RENDER_COMMAND(void)(
		[spoutName, textureRenderTarget2D](FRHICommandListImmediate& RHICmdList) {
			...
			FSpoutResource* ReciverResource = GetRegisteredSpout(spoutName);
...
void USpoutInterface::CloseReceiver(FString spoutName)
{
	if (IsSpoutResourceRegistered(spoutName)) 
	{
		if (GetRegisteredSpout(spoutName)->ReceiverRT != nullptr) GetRegisteredSpout(spoutName)->ReceiverRT->UpdateResource();
